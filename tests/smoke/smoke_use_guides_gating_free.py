...
# --- LESSON con use_guides=true (debe estar GATED en plan Free) ---
payload = {
...
rc, out, err = run(cmd)
if rc != 0:
    print("GUIDES_CURL_FAIL"); print(err or out); sys.exit(1)

body, status = split(out)
print("GUIDES_STATUS:", status)
print("GUIDES_BODY_HEAD:", body[:300].replace("\n", "\\n"))

# PASS esperado: 403 por gating de web_search
if status == "403":
    j = json.loads(body)
    detail = (j.get("detail") or "").lower() if isinstance(j, dict) else ""
    if ("guías" in detail or "guias" in detail) and (("pro" in detail) or ("premium" in detail) or ("solo" in detail)):
        print("SMOKE_OK: use_guides gated correctly for Free plan")
        sys.exit(0)
...
print("SMOKE_FAIL: use_guides gating incorrect (expected 403)")
print(body)
sys.exit(1)
